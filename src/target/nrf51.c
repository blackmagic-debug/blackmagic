...
static void nrf51_add_flash(target_s *target, uint32_t addr, size_t length, size_t erasesize)
{
	...
	flash->erase = nrf51_flash_erase;
	flash->write = nrf51_flash_write;
	flash->prepare = nrf51_flash_prepare;
	flash->done = nrf51_flash_done;
	flash->erased = 0xff;
...
bool nrf51_probe(target_s *target)
{
	...
	uint32_t uid0 = target_mem32_read32(target, NRF51_FICR_DEVICEID_LOW);
	uint32_t uid1 = target_mem32_read32(target, NRF51_FICR_DEVICEID_HIGH);
	if (uid0 == 0xffffffffU || uid1 == 0xffffffffU || uid0 == 0 || uid1 == 0)
		return false;
	/* Test for NRF52 device */
	uint32_t info_part = target_mem32_read32(target, NRF52_PART_INFO);
	if (info_part != 0xffffffffU && info_part != 0 && (info_part & 0x00ff000U) == 0x52000U) {
		uint32_t ram_size = target_mem32_read32(target, NRF52_INFO_RAM);
		target->driver = "nRF52";
		target->target_options |= TOPT_INHIBIT_NRST;
		target_add_ram32(target, 0x20000000U, ram_size * 1024U);
		nrf51_add_flash(target, 0, page_size * code_size, page_size);
		nrf51_add_flash(target, NRF51_UICR, page_size, page_size);
		target_add_commands(target, nrf51_cmd_list, "nRF52");
	} else {
		target->driver = "nRF51";
		/*
		 ...
		 */
		target_add_ram32(target, 0x20000000U, 0x8000U);
		target->target_options |= TOPT_INHIBIT_NRST;
		nrf51_add_flash(target, 0, page_size * code_size, page_size);
		nrf51_add_flash(target, NRF51_UICR, page_size, page_size);
		target_add_commands(target, nrf51_cmd_list, "nRF51");
	}
	target->mass_erase = nrf51_mass_erase;
	return true;
}

static bool nrf51_wait_ready(target_s *const target, platform_timeout_s *const print_progress)
{
	/* Poll for NVMC_READY */
	while (target_mem32_read32(target, NRF51_NVMC_READY) == 0) {
...
static bool nrf51_flash_done(target_flash_s *flash)
{
	...
	/* Return to read-only */
	target_mem32_write32(target, NRF51_NVMC_CONFIG, NRF51_NVMC_CONFIG_REN);
	return nrf51_wait_ready(target, NULL);
}
...
static bool nrf51_mass_erase(target_s *const target, platform_timeout_s *const print_progess)
{
	...
	/* Erase all */
	target_mem32_write32(target, NRF51_NVMC_ERASEALL, 1U);
...
static bool nrf51_cmd_erase_uicr(target_s *target, int argc, const char **argv)
{
	...
	/* Erase UICR */
	target_mem32_write32(target, NRF51_NVMC_ERASEUICR, 1U);
	return nrf51_wait_ready(target, NULL);
}
...
static bool nrf51_cmd_protect_flash(target_s *target, int argc, const char **argv)
{
	(void)argc;
	(void)argv;
	tc_printf(target, "Enabling Flash protection..\n");
	...
	target_mem32_write32(target, NRF51_APPROTECT, 0xffffff00U);
	return nrf51_wait_ready(target, NULL);
}
...
static bool nrf51_cmd_read_hwid(target_s *target, int argc, const char **argv)
{
	(void)argc;
	(void)argv;
	uint32_t hwid = target_mem32_read32(target, NRF51_FICR_CONFIGID) & 0xffffU;
	tc_printf(target, "Hardware ID: 0x%04" PRIX32 "\n", hwid);
...
static bool nrf51_cmd_read_fwid(target_s *target, int argc, const char **argv)
{
	(void)argc;
	(void)argv;
	uint32_t fwid = (target_mem32_read32(target, NRF51_FICR_CONFIGID) >> 16U) & 0xffffU;
	tc_printf(target, "Firmware ID: 0x%04" PRIX32 "\n", fwid);
...
static bool nrf51_cmd_read_deviceid(target_s *target, int argc, const char **argv)
{
	(void)argc;
	(void)argv;
	uint32_t deviceid_low = target_mem32_read32(target, NRF51_FICR_DEVICEID_LOW);
	uint32_t deviceid_high = target_mem32_read32(target, NRF51_FICR_DEVICEID_HIGH);

	tc_printf(target, "Device ID: 0x%08" PRIX32 "%08" PRIX32 "\n", deviceid_high, deviceid_low);
...
static bool nrf51_cmd_read_deviceinfo(target_s *target, int argc, const char **argv)
{
	...
	di.package = target_mem32_read32(target, NRF51_FICR_DEVICE_INFO_PACKAGE);
	di.part = target_mem32_read32(target, NRF51_FICR_DEVICE_INFO_PART);
	di.ram = target_mem32_read32(target, NRF51_FICR_DEVICE_INFO_RAM);
	di.flash = target_mem32_read32(target, NRF51_FICR_DEVICE_INFO_FLASH);
	di.variant.f = target_mem32_read32(target, NRF51_FICR_DEVICE_INFO_VARIANT);

	tc_printf(target, "Part:\t\tNRF%" PRIX32 "\n", di.part);
	tc_printf(target, "Variant:\t%c%c%c%c\n", di.variant.c[3], di.variant.c[2], di.variant.c[1], di.variant.c[0]);
	tc_printf(target, "Package:\t");
	switch (di.package) {
	case NRF51_FIELD_UNSPECIFIED:
		tc_printf(target, "Unspecified\n");
		break;
	case 0x2000U:
		tc_printf(target, "QF\n");
		break;
	case 0x2001U:
		...
		break;
	case 0x2004U:
		tc_printf(target, "QIxx\n");
		break;
	default:
		tc_printf(target, "Unknown (Code %" PRIX32 ")\n", di.package);
		break;
	}

	tc_printf(target, "Ram:\t\t%" PRIu32 "kiB\n", di.ram);
	tc_printf(target, "Flash:\t\t%" PRIu32 "kiB\n", di.flash);
	return true;
}
...
static bool nrf51_cmd_read_deviceaddr(target_s *target, int argc, const char **argv)
{
	(void)argc;
	(void)argv;
	uint32_t addr_type = target_mem32_read32(target, NRF51_FICR_DEVICEADDRTYPE);
	uint32_t addr_low = target_mem32_read32(target, NRF51_FICR_DEVICEADDR_LOW);
	uint32_t addr_high = target_mem32_read32(target, NRF51_FICR_DEVICEADDR_HIGH) & 0xffffU;

	if (!(addr_type & 1U))
		tc_printf(target, "Publicly Listed Address: 0x%04" PRIX32 "%08" PRIX32 "\n", addr_high, addr_low);
	else
		tc_printf(target, "Randomly Assigned Address: 0x%04" PRIX32 "%08" PRIX32 "\n", addr_high, addr_low);
...
static bool nrf51_cmd_read_help(target_s *target, int argc, const char **argv)
{
	...
	tc_printf(target, "Read commands:\n");
...
static bool nrf51_cmd_read(target_s *target, int argc, const char **argv)
{
	if (argc > 1) {
		for (const command_s *cmd = nrf51_read_cmd_list; cmd->cmd; ++cmd) {
			 ...
			 */
			if (!strncmp(argv[1], cmd->cmd, strlen(argv[1])))
				return cmd->handler(target, argc - 1, argv + 1);
		}
	}
...
bool nrf51_ctrl_ap_probe(adiv5_access_port_s *ap)
{
	...
	target->mass_erase = nrf51_ctrl_ap_mass_erase;
	adiv5_ap_ref(ap);
	target->priv = ap;
	...
	if (status)
		target->driver = "nRF52 Access Port";
	else
		target->driver = "nRF52 Access Port (protected)";
	target->regs_size = 0;

	return true;
}

static bool nrf51_ctrl_ap_mass_erase(target_s *const target, platform_timeout_s *const print_progess)
{
