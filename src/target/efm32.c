...
{
	return ((uint64_t)target_mem_read32(t, EFR32FG23_DI_EUI64H) >> 8U);
}
...
static uint64_t efr32fg23_read_unique(target_s *t, uint8_t di_version)
{
	...
	return (((uint64_t)target_mem_read32(t, EFR32FG23_DI_EUI64H) & 0x000FU) << 32U ) | target_mem_read32(t, EFR32FG23_DI_EUI64L);
}
...
static uint32_t efm32_flash_page_size(target_s *t, uint8_t di_version)
{
	...
	case 2U:
		...
		break;
	case 3U:
		mem_info_page_size = target_mem_read8(t, EFR32FG23_DI_MEMINFO);
		break;
	default:
		return 0;
	}
...
static uint16_t efm32_read_part_number(target_s *t, uint8_t di_version)
{
	...
	case 3:
		return target_mem_read32(t, EFR32FG23_DI_PART) & 0xffffU;
	default:
		return 0;
	}
}
...
static uint8_t efm32_read_part_family(target_s *t, uint8_t di_version)
{
	...
	case 3:
		return (target_mem_read32(t, EFR32FG23_DI_PART) >> 23U) & 0x3fU;
	default:
		return 0;
	}
}
...
bool efm32_probe(target_s *t)
{
	...
	uint32_t oui24 = ((efm32_v1_read_eui64(t) >> 40U) & 0xffffffU); 
	if (oui24 == EFM32_V1_DI_EUI_SILABS) {
		...
		di_version = 2;
	} else if (oui24 == EFR32FG23_DI_EUI_ENERGYMICRO) {
	...
	efm32_add_flash(t, 0x08000000, flash_size, flash_page_size);
...
static bool efm32_flash_write(target_flash_s *f, target_addr_t dest, const void *src, size_t len)
{
	...
	for (size_t i = 0; i < len; i+=4){
		target_mem_write32(t, EFM32_MSC_ADDRB(msc), (uint32_t)((uint32_t)dest + (uint32_t)i));
		target_mem_write32(t, EFM32_MSC_WDATA(msc), *(uint32_t*)((uint8_t*)src + (uintptr_t)i));
	}
