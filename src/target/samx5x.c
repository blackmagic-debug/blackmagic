...
const struct command_s samx5x_cmd_list[] = {
	{"lock_flash", (cmd_handler)samx5x_cmd_lock_flash, "Locks flash against spurious commands"},
	{"unlock_flash", (cmd_handler)samx5x_cmd_unlock_flash, "Unlocks flash"},
	{"lock_bootprot", (cmd_handler)samx5x_cmd_lock_bootprot, "Lock the boot protections to maximum"},
	{"unlock_bootprot", (cmd_handler)samx5x_cmd_unlock_bootprot, "Unlock the boot protections to minimum"},
	{"user_page", (cmd_handler)samx5x_cmd_read_userpage, "Prints user page from flash"},
	{"serial", (cmd_handler)samx5x_cmd_serial, "Prints serial number"},
	{"set_security_bit", (cmd_handler)samx5x_cmd_ssb, "Sets the security bit"},
	{"update_user_word", (cmd_handler)samx5x_cmd_update_user_word, "Sets 32-bits in the user page: <addr> <value>"},
#ifdef SAMX5X_EXTRA_CMDS
	{"mbist", (cmd_handler)samx5x_cmd_mbist, "Runs the built-in memory test"},
	{"write8", (cmd_handler)samx5x_cmd_write8, "Writes an 8-bit word: write8 <addr> <value>"},
	{"write16", (cmd_handler)samx5x_cmd_write16, "Writes a 16-bit word: write16 <addr> <value>"},
	{"write32", (cmd_handler)samx5x_cmd_write32, "Writes a 32-bit word: write32 <addr> <value>"},
#endif
	{NULL, NULL, NULL},
};
...
#define SAMX5X_NVM_SERIAL(n)   (0x0080600C + (n == 0 ? 0x1F0 : n * 4))
...
struct samx5x_descr samx5x_parse_device_id(uint32_t did)
{
	...
	uint8_t series = (did >> SAMX5X_DID_SERIES_POS) & SAMX5X_DID_SERIES_MASK;
	uint8_t revision = (did >> SAMX5X_DID_REVISION_POS) & SAMX5X_DID_REVISION_MASK;
	uint8_t devsel = (did >> SAMX5X_DID_DEVSEL_POS) & SAMX5X_DID_DEVSEL_MASK;
...
bool samx5x_probe(target *t)
{
	...
	if (protected) {
		snprintf(priv_storage->samx5x_variant_string, sizeof(priv_storage->samx5x_variant_string),
			"Microchip SAM%c%d%c%dA (rev %c) (PROT=1)", samx5x.series_letter, samx5x.series_number, samx5x.pin,
			samx5x.mem, samx5x.revision);
	} else {
		snprintf(priv_storage->samx5x_variant_string, sizeof(priv_storage->samx5x_variant_string),
			"Microchip SAM%c%d%c%dA (rev %c)", samx5x.series_letter, samx5x.series_number, samx5x.pin, samx5x.mem,
			samx5x.revision);
	}
...
static bool samx5x_flash_erase(target_flash_s *f, target_addr_t addr, size_t len)
{
	...
	flash_size = (target_mem_read32(t, SAMX5X_NVMC_PARAM) & 0xffff) * SAMX5X_PAGE_SIZE;
...
static bool samx5x_flash_write(target_flash_s *f, target_addr_t dest, const void *src, size_t len)
{
	...
	if (error || target_check_error(t) || samx5x_check_nvm_error(t)) {
		DEBUG_WARN("Error writing flash page at 0x%08" PRIx32 " (len 0x%08zx)\n", dest, len);
		return false;
	}
...
static int samx5x_write_user_page(target *t, uint8_t *buffer)
{
	uint16_t errs = samx5x_read_nvm_error(t);
	if (errs) {
		DEBUG_INFO(NVM_ERROR_BITS_MSG, "erase and write", (uint32_t)SAMX5X_NVM_USER_PAGE, (size_t)SAMX5X_PAGE_SIZE);
		samx5x_print_nvm_error(errs);
		samx5x_clear_nvm_error(t);
	}
...
{
	uint8_t factory_bits[] = {/* 0     8    16    24    32    40    48    56 */
...
static bool samx5x_cmd_lock_flash(target *t, int argc, const char **argv)
{
	...
	tc_printf(t,
		"Flash locked. The target must be reset for "
		"this to take effect.\n");
	return true;
}
...
static bool samx5x_cmd_unlock_flash(target *t, int argc, const char **argv)
{
	...
	tc_printf(t,
		"Flash unlocked. The target must be reset for "
		"this to take effect.\n");
	return true;
}
...
static bool samx5x_cmd_lock_bootprot(target *t, int argc, const char **argv)
{
	...
	tc_printf(t,
		"Bootprot locked. The target must be reset for "
		"this to take effect.\n");
	return true;
}
...
static bool samx5x_cmd_unlock_bootprot(target *t, int argc, const char **argv)
{
	...
	tc_printf(t,
		"Bootprot unlocked. The target must be reset for "
		"this to take effect.\n");
	return true;
}
...
static bool samx5x_cmd_update_user_word(target *t, int argc, const char **argv)
{
	...
	if (addr_end == argv[1] || (!addr && *(addr_end - 1) != '0') || value_end == argv[2] ||
		(!value && *(value_end - 1) != '0') || (argc == 4 && strcmp(argv[3], "force"))) {
...
static bool samx5x_cmd_mbist(target *t, int argc, const char **argv)
{
	...
	while (((status = target_mem_read32(t, SAMX5X_DSU_CTRLSTAT)) &
			   (SAMX5X_STATUSA_DONE | SAMX5X_STATUSA_PERR | SAMX5X_STATUSA_FAIL)) == 0)
		if (target_check_error(t))
			return false;
