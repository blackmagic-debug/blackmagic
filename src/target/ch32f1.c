...
static bool ch32f1_flash_busy_wait(target_s *const target)
{
	...
	while (status & FLASH_SR_BSY) {
		status = target_mem_read32(target, FLASH_SR);
...
{
	/* Read FLASH_SR to poll for EOP bit */
...
static void ch32f1_flash_set_cr(target_s *const target, const uint32_t bits)
{
	const uint32_t cr = target_mem_read32(target, FLASH_CR);
	target_mem_write32(target, FLASH_CR, cr | bits);
}

static void ch32f1_flash_clear_cr(target_s *const target, const uint32_t bits)
{
	const uint32_t cr = target_mem_read32(target, FLASH_CR);
	target_mem_write32(target, FLASH_CR, cr & ~bits);
}

static void ch32f1_flash_magic(target_s *const target, const uint32_t addr)
{
	const uint32_t magic = target_mem_read32(target, addr ^ MAGIC_WORD);
	target_mem_write32(target, FLASH_MAGIC, magic);
}
...
static bool ch32f1_flash_unlock(target_s *target)
{
	...
	uint32_t cr = target_mem_read32(target, FLASH_CR);
...
static bool ch32f1_flash_lock(target_s *target)
{
	 ...
	 */
	ch32f1_flash_set_cr(target, FLASH_CR_LOCK | FLASH_CR_FLOCK_CH32);
	const uint32_t cr = target_mem_read32(target, FLASH_CR);
	if (!(cr & FLASH_CR_FLOCK_CH32))
...
bool ch32f1_probe(target_s *target)
{
	...
	uint32_t signature = target_mem_read32(target, FLASHSIZE);
	...
	ch32f1_add_flash(target, FLASH_BEGIN_ADDRESS_CH32, flashSize * 1024U, 128);
...
bool ch32f1_flash_erase(target_flash_s *flash, target_addr_t addr, size_t len)
{
	...
	while (len) {
		ch32f1_flash_set_cr(target, FLASH_CR_FTER_CH32); // CH32 PAGE_ER
		/* Write address to FMA */
		target_mem_write32(target, FLASH_AR, addr);
		/* Flash page erase start instruction */
		ch32f1_flash_set_cr(target, FLASH_CR_STRT);
		if (!ch32f1_flash_eop_wait(target))
			return -1;
		target_mem_write32(target, FLASH_SR, FLASH_SR_EOP); /* Clear EOP flag */
		ch32f1_flash_clear_cr(target, FLASH_CR_STRT);
		// Magic
		ch32f1_flash_magic(target, addr);
		if (len > 128U)
			len -= 128U;
		else
			len = 0;
	...
	const uint32_t sr = target_mem_read32(target, FLASH_SR);
...
static int ch32f1_upload(target_s *target, uint32_t dest, const void *src, uint32_t offset)
{
	...
	if (!ch32f1_flash_eop_wait(target))
		return -1;
	target_mem_write32(target, FLASH_SR, FLASH_SR_EOP); /* Clear EOP flag */
	ch32f1_flash_clear_cr(target, FLASH_CR_FTPG_CH32);
	ch32f1_flash_magic(target, dest + offset);
	return 0;
}
...
static int ch32f1_buffer_clear(target_s *target)
{
	...
	if (!ch32f1_flash_busy_wait(target))
		return -1;
	ch32f1_flash_clear_cr(target, FLASH_CR_FTPG_CH32); // Fast page program 4-
	return 0;
}
...
static bool ch32f1_flash_write(target_flash_s *flash, target_addr_t dest, const void *src, size_t len)
{
	...
	while (length > 0) {
		...
		if (!ch32f1_wait_flash_ready(target, dest))
			return false;

		for (size_t i = 0; i < 8U; i++) {
			if (ch32f1_upload(target, dest, src, i * 16U)) {
		...
		// write buffer
		ch32f1_flash_set_cr(target, FLASH_CR_FTPG_CH32);
		target_mem_write32(target, FLASH_AR, dest); // 10
		ch32f1_flash_set_cr(target, FLASH_CR_STRT); // 11 Start
		if (!ch32f1_flash_eop_wait(target))         // 12
			return -1;
		target_mem_write32(target, FLASH_SR, FLASH_SR_EOP); /* Clear EOP flag */
		ch32f1_flash_clear_cr(target, FLASH_CR_FTPG_CH32);

		ch32f1_flash_magic(target, dest);
		...
		const uint32_t sr = target_mem_read32(target, FLASH_SR); // 13
		ch32f1_flash_lock(target);
		if (sr & SR_ERROR_MASK) {
