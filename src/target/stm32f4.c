...
static bool stm32f4_attach(target_s *const target)
{
	...
	if (large_sectors) {
		...
		if (bank_length > 0x10000U) {
			stm32f4_add_flash(target, AXIM_BASE + 0x10000, 0x10000, 0x10000, 4, split);
			if (remaining_bank_length)
				stm32f4_add_flash(target, AXIM_BASE + 0x20000, remaining_bank_length, 0x20000, 5, split);
		}
		...
		if (is_f7) {
			...
			if (bank_length > 0x10000U) {
				...
				if (remaining_bank_length)
					stm32f4_add_flash(target, ITCM_BASE + 0x20000, remaining_bank_length, 0x20000, 5, split);
			}
		}
