 ...
 *
 * Copyright (C) 2022 hardesk <hardesk@gmail.com>
 *
...
#define MSPM0L_FLASH_NONMAIN	0x41c00000U /* One Sector, BANK0. Device boot configuration (BCR, BSL) */
#define MSPM0L_FLASH_FACTORY	0x41c40000U	/* One Sector, BANK0. Non Modifiable */
#define MSPM0L_FLASH_DATA		0x41d00000U
#define MSPM0L_FLASH_SECTOR_SZ	1024

#define MSPM0L_FLASHCTL_BASE		0x400cd000
#define MSPM0L_FLASHCTL_CMDEXEC		(MSPM0L_FLASHCTL_BASE + 0x1100)
#define MSPM0L_FLASHCTL_CMDTYPE		(MSPM0L_FLASHCTL_BASE + 0x1104)
#define MSPM0L_FLASHCTL_CMDCTL		(MSPM0L_FLASHCTL_BASE + 0x1108)
#define MSPM0L_FLASHCTL_CMDADDR		(MSPM0L_FLASHCTL_BASE + 0x1120)
#define MSPM0L_FLASHCTL_BYTEN		(MSPM0L_FLASHCTL_BASE + 0x1124)
#define MSPM0L_FLASHCTL_CMDDATAIDX	(MSPM0L_FLASHCTL_BASE + 0x112c)
#define MSPM0L_FLASHCTL_STATCMD		(MSPM0L_FLASHCTL_BASE + 0x13d0)

#define MSPM0L_FLASHCTL_CMDDATA0	(MSPM0L_FLASHCTL_BASE + 0x1130)
...
typedef struct mspm0l_flash {
	target_flash_s target_flash;
	uint8_t banks;
} mspm0l_flash_s;

static bool mspm0l_flash_erase(target_flash_s *flash, target_addr_t addr, size_t length);
static bool mspm0l_flash_write(target_flash_s *flash, target_addr_t dest, const void *src, size_t length);
static bool mspm0l_mass_erase(target_s *target);

static bool mspm0l_dump_factory_config(target_s *const target, const int argc, const char **const argv);
static bool mspm0l_dump_bcr_config(target_s *const target, const int argc, const char **const argv);

static command_s mspm0l_cmds_list[] = {
	{"dump_factory", mspm0l_dump_factory_config, "Display FACTORY registers"},
	{"dump_bcr", mspm0l_dump_bcr_config, "Display NONMAIN (BCR/BSL) registers"},
	/* {"write_nm", mspm0l_write_non_main, "Write data to NONMAIN (configuration) flash. write_nm <address> <value>"}, */
	{NULL, NULL, NULL}
};
...
static confreg_s mspm0l_factory_regs[] =
{
	{ 0x00, 1, "TRACEID" },
	{ 0x04, 1, "DEVICEID" },
	{ 0x08, 1, "USERID" },
...
static confreg_s mspm0l_bcr_regs[] =
{
	{ 0x00, 1, "BCRCONFIGID" },
	{ 0x04, 1, "BOOTCFG0" },
	{ 0x08, 1, "BOOTCFG1" },
	...
	target_s *const target, const uint32_t base, const size_t length, size_t banks)
{
	...
	target_flash->blocksize = MSPM0L_FLASH_SECTOR_SZ;
	target_flash->writesize = 8;
	target_flash->erase = mspm0l_flash_erase;
	target_flash->write = mspm0l_flash_write;
...
bool mspm0l_probe(target_s *const target)
{
	uint32_t const deviceid = target_mem32_read32(target, MSPM0L_FLASH_FACTORY + 0x4);
	...
	DEBUG_TARGET("%s: Device %" PRIx32 "/%" PRIx32 " rev. %" PRIu32 ": user ver. %" PRIu32 ".%" PRIu32 ".%" PRIu32 " part %" PRIu32 "\n",
		__func__,
		BITS(11, 1, deviceid), BITS(27, 12, deviceid), BITS(31, 28, deviceid),
	...
	target_add_ram32(target, MSPM0L_SRAM_BASE, sram_size);

	mspm0l_add_flash(target, MSPM0L_FLASH_MAIN, mainflash_size, main_num_banks);
	if (dataflash_size != 0)
		mspm0l_add_flash(target, MSPM0L_FLASH_DATA, dataflash_size, 1);

	target_add_commands(target, mspm0l_cmds_list, "MSPM0L");
...
{
	(void)argc; (void)argv;
	mspm0l_dump_regs(target, mspm0l_factory_regs, MSPM0L_FLASH_FACTORY);
	return true;
}
...
{
	(void)argc; (void)argv;
	mspm0l_dump_regs(target, mspm0l_bcr_regs, MSPM0L_FLASH_NONMAIN);
	return true;
}
...
static void mspm0l_flash_unprotect(target_flash_s *const target_flash)
{
	target_mem32_write32(target_flash->t, MSPM0L_FLASHCTL_CMDWEPROTA, 0);
	target_mem32_write32(target_flash->t, MSPM0L_FLASHCTL_CMDWEPROTB, 0);
	target_mem32_write32(target_flash->t, MSPM0L_FLASHCTL_CMDWEPROTC, 0);
}
	...
	const void *const src, const size_t length __attribute__((unused)) )
{
	...
	mspm0l_flash_unprotect_sector(target_flash, dest);
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDTYPE, MSPM0L_FLASH_CMDTYPE_PROG | MSPM0L_FLASH_CMDTYPE_SZ_1WORD);
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDCTL, 0);
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDADDR, dest);
	target_mem32_write32(target, MSPM0L_FLASHCTL_BYTEN, 0xffffffff);
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDDATA0, read_le4((const uint8_t *)src, 0));
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDDATA1, read_le4((const uint8_t *)src, 4));
	target_mem32_write32(target, MSPM0L_FLASHCTL_CMDEXEC, 1);
...
static bool mspm0l_mass_erase(target_s *const target)
{
	platform_timeout_s timeout;
	platform_timeout_set(&timeout, 500);
		...
		flash = (mspm0l_flash_s*)flash->target_flash.next)
	{
		...
		target_mem32_write32(target, MSPM0L_FLASHCTL_CMDCTL, 0);
		target_mem32_write32(target, MSPM0L_FLASHCTL_CMDADDR, flash->target_flash.start);
		target_mem32_write32(target, MSPM0L_FLASHCTL_CMDEXEC, 1);
