...
char __attribute__((aligned(0x04))) stub_stack[0x20];

void mspm0_flash_writer(const uint32_t *const dest, const uint32_t *const src, const uint32_t size)
{
	for (uint32_t i = 0U; i < size / 4U; i += 2U) {
...
void __attribute__((naked, used, section(".entry"))) mspm0_flash_write_entry(
	const uint32_t *const dest, const uint32_t *const src, const uint32_t size)
{
	__asm volatile("msr msp, %0" : : "r"(stub_stack + sizeof(stub_stack)) :);
	mspm0_flash_writer(dest, src, size);
}