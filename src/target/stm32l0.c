...
static bool stm32lx_cmd_option(target_s *const target, const int argc, const char **const argv)
{
	...
	if (stm32lx_is_stm32l1(target)) {
		tc_printf(target,
			...
			options, (uint32_t)read_protection, (options & STM32L1_NVM_OPTR_SPRMOD) ? 1 : 0,
		...
		tc_printf(target, "OPTR: 0x%08" PRIx32 ", RDPROT %" PRIu32 ", WPRMOD %" PRIu16 ", WDG_SW %u, BOOT1 %u\n",
			options, (uint32_t)read_protection, (options & STM32L0_NVM_OPTR_WPRMOD) ? 1 : 0,
			(options & STM32Lx_NVM_OPTR_WDG_SW) ? 1 : 0, (options & STM32L0_NVM_OPTR_BOOT1) ? 1 : 0);
	}
...
usage:
	...
	tc_printf(target, "The value of <addr> must in the interval [0x%08" PRIx32 ", 0x%" PRIx32 ")\n",
		STM32Lx_NVM_EEPROM_PHYS, STM32Lx_NVM_EEPROM_PHYS + stm32lx_nvm_eeprom_size(target));
