...
void gdb_tcp_server(void)
{
	...
	case sm_home:
		...
		int8_t result = bind(gdb_server_socket, (struct sockaddr *)&gdb_addr, sizeof(gdb_addr));
...
static void app_wifi_callback(uint8_t msgType, void *pvMsg)
{
	(void)pvMsg;
	switch (msgType) {
	...
	case M2M_WIFI_CONN_STATE_CHANGED_EVENT: {
		...
		if (pstrWifiState->u8CurrState == M2M_WIFI_CONNECTED) {
			...
			//
			if (http_active == true && waiting_access_point == true) {
	...
	case M2M_SOCKET_RECV_EVENT: {
		...
		if (sock == gdb_client_socket) {
			...
			if (pRecvData->bufSize > 0) {
				...
				//
				int local_count = pRecvData->bufSize;
				//
				// Copy data to circular input buffer
				//
				for (int i = 0; local_count != 0;
			...
			if (pRecvData->bufSize > 0) {
				if (user_configured_uart == false) {
					if (platform_configure_uart((char *)&local_uart_debug_buffer[0]) == false) {
						//
...
void app_task(void)
{
	...
	case app_state_wait_wps_event: {
		...
		} else if (wps_active == false) // Did WPS timeout?
		{
			mode_task_state = mode_led_idle_state;
			led_mode = mode_led_idle;
			app_state = app_state_spin;
		} else if (tim2_is_seconds_timeout() == true) // Did we timeout waiting for WPS?
		{
			/*
				Cancel WPS mode
				*/
	...
	case app_state_wait_provision_event: {
		if (is_wifi_connected() == true) {
			...
			//
			led_mode = mode_led_ap_connected;
		} else if (http_active == false) // Did provisioning timeout?
		{
			mode_task_state = mode_led_idle_state;
			led_mode = mode_led_idle;
			app_state = app_state_spin;
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
	...
	case app_state_spin: {
		...
		//
		if (new_gdb_client_connected == true) {
		...
		if (new_uart_debug_client_conncted == true) {
			new_uart_debug_client_conncted = false;
			recv(uart_debug_client_socket, &local_uart_debug_buffer[0], UART_DEBUG_INPUT_BUFFER_SIZE, 0);
		}
		if (new_swo_trace_client_conncted == true) {
...
void send_swo_trace_data(uint8_t *lpBuffer, uint8_t length)
{
	...
	m2mStub_EintDisable();
	memcpy(swo_trace_send_queue[swo_trace_send_queue_in].packet, lpBuffer, length);
