...
void trace_buf_drain(usbd_device *dev, uint8_t ep)
{
	...
	if (write_index != read_index) {
		...
		if (decoding)
			...
			result = traceswo_decode(dev, CDCACM_UART_ENDPOINT, &trace_rx_buf[read_index * FULL_SWO_PACKET], FULL_SWO_PACKET);
		else
...
void SWO_DMA_ISR(void)
{
	if (DMA_ISR(SWO_DMA_BUS) & DMA_ISR_HTIF(SWO_DMA_CHAN)) {
		...
		memcpy(&trace_rx_buf[write_index * FULL_SWO_PACKET], pingpong_buf, FULL_SWO_PACKET);
	}
